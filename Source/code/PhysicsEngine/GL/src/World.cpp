...
World::World(std::string m, std::string windowName)
{
	mode = m;
	controls = NULL;
    ...
    if (mode == "VERBOSE")
    {
		...
		string vertex = string(getenv("SHADER_PATH")) + string("SimpleVertexShader.vertexshader");
		string frag   = string(getenv("SHADER_PATH")) + string("SimpleFragmentShader.vertexshader");
		defaultShader = loadShaders(vertex.c_str(), frag.c_str());
